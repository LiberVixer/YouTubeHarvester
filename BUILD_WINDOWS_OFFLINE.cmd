...
if not exist "%WHEELHOUSE%" (
    ...
    echo   YouTubeHarvester-1.0.0-offline\wheelhouse
    echo   YouTubeHarvester-1.0.0-offline\source
    exit /b 1
)
...
if not exist "%FFMPEG_DIR%\ffmpeg.exe" if not exist "%FFMPEG_DIR%\bin\ffmpeg.exe" (
    ...
    echo   YouTubeHarvester-1.0.0-offline\ffmpeg\bin\ffmpeg.exe
    echo   YouTubeHarvester-1.0.0-offline\ffmpeg\bin\ffprobe.exe
    echo.
...
if not exist "%DENO_DIR%\deno.exe" if not exist "%DENO_DIR%\bin\deno.exe" (
    ...
    echo   YouTubeHarvester-1.0.0-offline\deno\deno.exe
    echo.
...
copy /Y "%SOURCE_DIR%dist\release\YouTubeHarvester_1.0.0_windows*" "%WINDOWS_RELEASE%\" >nul 2>nul
