...
if not exist "%WHEELHOUSE%" (
    ...
    echo   YouTubeHarvester-1.1.2-offline\wheelhouse
    echo   YouTubeHarvester-1.1.2-offline\source
    exit /b 1
)
...
if not exist "%FFMPEG_DIR%\ffmpeg.exe" if not exist "%FFMPEG_DIR%\bin\ffmpeg.exe" (
    ...
    echo   YouTubeHarvester-1.1.2-offline\ffmpeg\bin\ffmpeg.exe
    echo   YouTubeHarvester-1.1.2-offline\ffmpeg\bin\ffprobe.exe
    echo.
...
if not exist "%DENO_DIR%\deno.exe" if not exist "%DENO_DIR%\bin\deno.exe" (
    ...
    echo   YouTubeHarvester-1.1.2-offline\deno\deno.exe
    echo.
...
copy /Y "%SOURCE_DIR%dist\release\YouTubeHarvester_1.1.2_windows*" "%WINDOWS_RELEASE%\" >nul 2>nul
