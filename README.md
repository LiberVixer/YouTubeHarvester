...
<p align="center">
  ...
  Linux uses the stable Bash engine; Windows beta builds ship with a portable Python engine.
  <br>
...
**YouTube Harvester** is a compact PyQt5 desktop app for Linux Mint, Ubuntu, Windows, and similar desktop systems. It sits in the system tray, watches selected YouTube channels, downloads new videos through `yt-dlp`, keeps a manual queue, and can publish downloaded items to a Telegram channel. Linux uses the stable Bash downloader by default; Windows beta packages use the portable Python engine with bundled `ffmpeg`, `ffprobe`, and `deno`.
...
- Download engine selector on Linux: stable Bash engine by default, Python engine for portability testing.
...
```bash
sudo apt install ./yt-harvester_0.2.4~beta_all.deb
```
...
```bash
packaging/build_release.sh 0.2.4~beta 0.2.4-beta
```

The release files will be written to `dist/release`:

- `YouTubeHarvester_0.2.4-beta_linux_all.deb`
- `YouTubeHarvester_0.2.4-beta_source.tar.gz`
...
The Python downloader engine is the Windows beta path. The Bash engine remains the default and recommended path on Linux.
...
**YouTube Harvester** — небольшая программа для Linux Mint, Ubuntu, Windows и похожих настольных систем. Она живёт в трее, следит за выбранными YouTube-каналами, скачивает новые ролики через `yt-dlp`, умеет работать с ручной очередью и отправлять найденные видео в Telegram-канал. На Linux по умолчанию используется стабильный Bash-движок, а Windows beta-сборки работают через переносимый Python-движок с bundled `ffmpeg`, `ffprobe` и `deno`.
...
- Выбор движка скачивания на Linux: стабильный Bash по умолчанию и Python для тестов переносимости.
...
Python-движок скачивания используется в Windows beta-сборках. Для обычного использования на Linux по умолчанию остаётся Bash-движок.
